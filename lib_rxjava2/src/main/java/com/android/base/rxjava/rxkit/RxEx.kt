...
/**
 *  RxJava2 的一个重要的设计理念是：不吃掉任何一个异常。产生的问题是，当 RxJava2 “downStream” 取消订阅后，“upStream” 仍有可能抛出异常，
 *  这时由于已经取消订阅， * “downStream” 无法处理异常，此时的异常无人处理，便会导致程序崩溃,解决方案：在 Application 设置 RxJavaPlugin 的 ErrorHandler。
 *
 * refer: [RxJava2 使用过程中遇到的坑](https://github.com/qqiabc521/blog/issues/3)
 */
...
inline fun <reified A : Any> RxBus.toObservable(): Flowable<A> = this.toFlowable(A::class.java)
inline fun <reified A : Any> RxBus.toObservable(tag: String): Flowable<A> =
    this.toFlowable(tag, A::class.java)
