...
/**
 * 不要大范围的使用 RxBus，比较推荐的模式为针对特定模块定义特定的事件分发类：
 *
 * <pre>
 *     {@code
...
public class RxBus {
    ...
    public <T> Flowable<T> toFlowable(Class<T> tClass) {
        return toFlowable(COMMON_EVENT_IDENTIFY, tClass);
    }
    ...
    public <T> Flowable<T> toFlowable(@NonNull final String identify, final Class<T> tClass) {
