...
public class RxBusTest {
    ...
    public void start() {
        ...
        mRxBus.toFlowable(A.class).subscribe(a -> {
            System.out.println(a);
            mA = a;
        });

        mRxBus.toFlowable("A", A.class)
                .subscribe(a -> {
                    System.out.println(" has id " + a);
                    mA1 = a;
                });

        mRxBus.toFlowable(B.class).subscribe(b -> {
            System.out.println(b);
            mB = b;
        });

        mRxBus.toFlowable("B", B.class)
                .subscribe(b -> {
                    System.out.println(" has id " + b);
                    mB1 = b;
                });

        mRxBus.toFlowable(C.class).subscribe(c -> {
            System.out.println(c);
            mC = c;
        });

        mRxBus.toFlowable("C", C.class)
                .subscribe(c -> {
